...
// ============================================
// plan_course_llm — direct ureq LLM call
...
pub async fn plan_course_llm(
    ...
    // inject cross-course learner memory (best-effort — missing
    // index / corrupt profiles degrade to None, i.e. the legacy prompt).
    ...
    // 画像段落——read-only 缓存注入，缺文件/损坏/关闭 → None
    // （prompt 与 字节级一致）。规划链路绝不调 LLM 重建画像。
...
// ============================================
// rank_learner_courses — relevance ranking for the
...
// ============================================
// get_learner_persona — derived persona (domains + analogy bank).
...
// ============================================
// generate_roadmap — direct ureq LLM call
...
// ============================================
// Learning Elements, Quiz, Explanation
// ============================================
...
/// extras are no longer appended to standard, making them independent.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Build an extra quiz question from a concept + its AI explanation
...
/// Read quiz questions from pre-generated .quiz.json (refactored: no real-time AI)
/// generate extra questions based on explanations/<chapter>.json
/// extras returned separately from standard quiz questions.
#[tauri::command]
pub async fn generate_chapter_quiz(
    ...
    log::info!("generate_chapter_quiz for: {}", chapter_file);
    ...
    log::info!("reading quiz.json: {}", quiz_path);
        ...
        match std::fs::read_dir(p) {
            Ok(entries) => {
                let files: Vec<String> = entries
                    ...
                    .collect();
                log::info!("files in dir {:?}: {:?}", p, files);
            }
            Err(e) => log::warn!("cannot read dir {:?}: {}", p, e),
        }
    }
    ...
    let quiz_json: serde_json::Value = match serde_json::from_str(&quiz_content) {
        Ok(v) => v,
        Err(_) => {
            log::warn!("quiz.json parse failed, attempting heuristic repair");
    ...
    // read explanation cues and build separate extra questions
    // extras are returned alongside (not appended to) standard questions.
    ...
    if let Some(proj) = project_path.as_ref().filter(|p| !p.is_empty()) {
        if let Some(basename) = std::path::Path::new(&chapter_file).file_name() {
            ...
            log::info!(
                "checking explanations for extra quiz: {:?} exists={}",
                exp_path,
                exp_path.exists()
            );
            if exp_path.exists() {
                            ...
                            Ok(exp_data) => {
                                ...
                                let extras: Vec<(String, String)> = exp_data.conversations
                                    ...
                                    .filter_map(|c| {
                                        ...
                                        if already_covered {
                                            log::info!("skipping extra for '{}': already in quiz", concept);
                                            return None;
                                        }
                                    ...
                                    .collect();
                                log::info!(
                                    "selected {} extras from conversations",
                                    extras.len()
                                );
                                for (idx, (concept, explanation)) in extras.iter().enumerate() {
                                    ...
                                    log::info!(
                                        "built extra question for concept: {}",
                                        concept
                                    );
                                }
                            }
                            Err(e) => {
                                log::warn!("failed to parse explanations json: {}", e)
                            }
                        }
                    }
                    Err(e) => log::warn!("failed to read explanations file: {}", e),
                }
            }
        } else {
            log::warn!(
                "could not extract basename from chapter_file: {}",
                chapter_file
            );
        }
    } else {
        log::info!("project_path is empty or None, skipping extra questions");
    }
    ...
    log::info!(
        "loaded {} standard questions + {} extra questions",
...
pub async fn evaluate_quiz(
    ...
    log::info!("evaluate_quiz for: {}", chapter);
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        // decision: Agent failure → explicit error, no silent fallback
...
// ============================================
// PB1/PB2: explain_selection
// Direct ureq LLM call (replaced agent-bridge approach)
// ============================================
...
pub async fn explain_selection(
    ...
    log::info!("explain_selection START: text_len={}", text.len());
    ...
    if sdk_available {
        ...
        {
            Ok(result) => {
                log::info!("explain_selection via Agent SDK: ok");
                return Ok(result);
            }
            Err(e) => {
                log::info!(
                    "explain_selection Agent failed, falling back to ureq: {}",
                    e
                );
            }
        }
    } else {
        log::info!("Agent SDK not available, using ureq directly");
    }
    ...
    log::info!(
        "explain_selection SUCCESS: explanation_len={}, questions={}",
...
/// Adapt subsequent chapters based on quiz result
...
// ============================================
// Explanation Persistence Commands
// ============================================
...
pub async fn ensure_extra_questions(
    ...
    if legacy_extras.exists() {
        ...
        log::info!("removed legacy extras file {:?}", legacy_extras);
    }
    ...
    for conv in &exp_data.conversations {
        ...
        .await
        {
            Ok(()) => log::info!("generated extras for cue {}", conv.id),
            Err(e) => log::warn!(
                "failed to generate extras for cue {}: {}",
                conv.id,
                e
            ),
        }
...
pub async fn socratic_chat(
    ...
    log::info!(
        "socratic_chat START: project={}, concepts={:?}, has_answer={}, session_id={}",
        project_path,
        concept_titles,
        user_answer.is_some(),
        session_id.as_deref().unwrap_or("(none)")
    ...
    let config = crate::get_config(app_handle).map_err(|e| e.to_string())?;
    let bridge_path = get_agent_bridge_path()?;
    log::info!("socratic_chat: bridge_path={:?}", bridge_path);
    ...
    log::info!("socratic_chat: payload={}", payload);
    ...
    log::info!("socratic_chat: spawning node process...");
    let output = cmd.output().map_err(|e| {
        log::error!("socratic_chat: spawn failed: {}", e);
        format!("Failed to spawn agent-bridge: {}", e)
    })?;

    log::info!(
        "socratic_chat: exit_code={:?}, stdout_len={}, stderr_len={}",
        output.status.code(),
        output.stdout.len(),
        output.stderr.len()
    );
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        log::error!("socratic_chat: agent failed: stderr={}", stderr);
        return Err(format!("Agent socratic review failed: {}", stderr));
    }

    let stdout = String::from_utf8_lossy(&output.stdout);
    log::info!("socratic_chat: raw_stdout={}", stdout.trim());
    ...
    let result: crate::SocraticChatResponse = stdout
        ...
        .ok_or_else(|| {
            log::error!(
                "socratic_chat: no parseable result line — raw: {}",
                stdout.trim()
            );
    ...
    log::info!(
        "socratic_chat SUCCESS: content_len={}, done={}",
        result.content.len(),
        result.done
    );
    Ok(result)
...
/// Case Study via Agent SDK (agent-bridge "case-study" stage).
...
pub async fn case_study_chat(
    ...
    log::info!(
        "case_study_chat START: project={}, selected={:?}, first_turn={}, session_id={}",
        project_path,
        selected_text,
        user_answer.is_none(),
        session_id.as_deref().unwrap_or("(none)")
    ...
    // 流式输出：spawn + 逐行读 stdout。bridge 的 text_delta 经
    // {type:"case_study_delta"} JSON 行实时到达，逐行转发给前端渲染；
    // 最终结果行（SocraticChatResponse）在进程结束后解析。
    let mut child = cmd.spawn().map_err(|e| {
        log::error!("case_study_chat: spawn failed: {}", e);
        format!("Failed to spawn agent-bridge: {}", e)
    })?;
        ...
        match serde_json::from_str::<serde_json::Value>(trimmed) {
            Ok(event) if event.get("type").and_then(|v| v.as_str()) == Some("case_study_delta") => {
                if let Err(e) = app_handle.emit("case-study-event", &event) {
                    log::warn!("case-study-event emit failed: {}", e);
                }
            }
            _ => {
    ...
    if !status.success() {
        let stderr = child
            ...
            .unwrap_or_default();
        log::error!(
            "case_study_chat: agent failed: stderr={}",
            stderr
        );
...
// ============================================
// Exploration Mode Chat via Agent SDK
// ============================================
...
pub async fn explore_chat(
    ...
    log::info!("explore_chat START: article_len={}, history_len={}, message_len={}, session_id={}",
        article.len(), history.len(), message.len(), session_id.as_deref().unwrap_or("(none)"));

    let config = crate::get_config(app_handle.clone()).map_err(|e| e.to_string())?;
    let bridge_path = get_agent_bridge_path()?;
    log::info!("explore_chat: bridge_path={:?}", bridge_path);
    ...
    log::info!("explore_chat: spawning node process...");
    let output = cmd.output().map_err(|e| {
        log::error!("explore_chat: spawn failed: {}", e);
        format!("Failed to spawn agent-bridge: {}", e)
    })?;

    log::info!(
        "explore_chat: exit_code={:?}, stdout_len={}, stderr_len={}",
        output.status.code(),
        output.stdout.len(),
        output.stderr.len()
    );
    ...
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr);
        log::error!("explore_chat: agent failed: stderr={}", stderr);
        return Err(format!("Agent exploration chat failed: {}", stderr));
    }

    let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
    log::info!("explore_chat: raw_stdout_len={}", stdout.len());
    ...
    log::info!(
        "explore_chat SUCCESS: response_len={}",
        stdout.len()
    );
    Ok(stdout)
}
...
pub async fn generate_paper_reader_guide(
    ...
    log::info!(
        "generate_paper_reader_guide START: paper_file={}",
        paper_file
    );
    ...
    if output_file.exists() {
        log::info!(
            "paper-reader: cache hit, reading {}",
            output_file.display()
        );
        ...
        log::info!(
            "paper-reader cache hit SUCCESS: title={}",
            guide
                .get("title")
                .and_then(|t| t.as_str())
                .unwrap_or("(none)")
    ...
    if let Err(e) = copy_bundled_skills_to_project(&paper_dir.to_string_lossy()) {
        log::warn!(
            "paper-reader: failed to copy bundled skills: {}",
            e
        );
    }
    ...
    let generate_result = async {
        ...
        log::info!("paper-reader: spawning node process...");
        let output = cmd.output()
            .map_err(|e| {
                log::error!("paper-reader: spawn failed: {}", e);
                format!("Failed to spawn agent-bridge: {}", e)
            })?;

        log::info!("paper-reader: exit_code={:?}, stdout_len={}, stderr_len={}",
            output.status.code(), output.stdout.len(), output.stderr.len());

        if !output.status.success() {
            let stderr = String::from_utf8_lossy(&output.stderr);
            log::error!("paper-reader: agent failed: stderr={}", stderr);
        ...
        log::info!("paper-reader SUCCESS: title={}", guide.get("title").and_then(|t| t.as_str()).unwrap_or("(none)"));
        Ok(guide)
    }.await;
...
pub async fn submit_paper_reader_feedback(
    ...
    log::info!(
        "submit_paper_reader_feedback START: paper_file={}",
        paper_file
    );
    ...
    log::info!(
        "submit_paper_reader_feedback SUCCESS: file={}",
